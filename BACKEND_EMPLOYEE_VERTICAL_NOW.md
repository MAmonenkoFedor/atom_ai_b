...
Этот файл — **короткий handoff "отправить в чат бэку"**: статус интеграции, открытые вопросы и критерии готовности. Детальный API-контракт по полям и путям остается в `BACKEND_HANDOFF.md` (§5.2.1 и связанные разделы).

Сопутствующие документы (канон, без дублей):

| Документ | Зачем |
|----------|--------|
| `BACKEND_HANDOFF.md` | Канонические пути, alias, схемы, ошибки |
| `FRONTEND_LIVE_CONNECTION_GUIDE.md` | Env, креды, CSRF, smoke-чеклист, примеры payload |
| `FRONTEND_BACKEND_SYNC_PROTOCOL.md` | Как передавать изменения API и сигнал готовности endpoint |
...
Фронт шлет write-запросы в **snake_case** и нормализует ответы в `camelCase` на своей стороне.
...
Детали полей и примеры — в `BACKEND_HANDOFF.md` и в `FRONTEND_LIVE_CONNECTION_GUIDE.md`.
...
2. Фронт в `VITE_API_SOURCE=live` и `VITE_API_MOCK_FALLBACK=false` проходит smoke из `FRONTEND_LIVE_CONNECTION_GUIDE.md`.
3. OpenAPI соответствует `BACKEND_HANDOFF.md`.

## Следующий шаг для backend-команды

1. Закрыть alignment по трем открытым вопросам выше.
2. Отправить в чат **readiness signal** по каждому измененному endpoint (шаблон в `FRONTEND_BACKEND_SYNC_PROTOCOL.md`).
